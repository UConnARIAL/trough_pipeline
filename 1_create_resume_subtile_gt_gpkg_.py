#!/usr/bin/env python3
"""
gt_gpkg_common.py, Refactored to remove duplicated code while keeping stage-specific logic in
...
# -------------------- main --------------------
def main():
    p = argparse.ArgumentParser("Subtile-only TCN GPKG builder (per-file outputs)")
    p.add_argument("--config", required=True, help="Path to TOML config")
    p.add_argument("--workers", type=int, default=None, help="Override worker count (else from config/auto)")
    p.add_argument("--one-tile", type=str, default=None, help="Only process this tile_id subfolder")
    p.add_argument("--limit-tifs", type=int, default=0, help="Max number of pending TIFs to process (0=no limit)")
    ...
    logging.info("=== subtile gt_gpkg startup ===")
    logging.info(f"Python: {sys.version.split()[0]} | SciPy fast-path: {'ON' if HAVE_SCIPY else 'OFF'}")

    # keep your existing sanity checks (if it expects imagery_dir, pass master_dir twice or adapt that function)
    # If sanity_check_paths(master_dir, imagery_dir) is strict, update it OR create a new subtile-only checker.
    # Here: we only require master_dir exists.
    if not os.path.isdir(master_dir):
        logging.error(f"master_dir not found: {master_dir}")
        return
    ...
    for tile_id in tiles:
        if remaining <= 0:
            break
        tif_paths = glob.glob(os.path.join(master_dir, tile_id, "*.tif"))
        if not tif_paths:
            continue
        ...
        take = len(tif_paths) if remaining is None else min(remaining, len(tif_paths))
        for tif in sorted(tif_paths[:take]):
    ...
    # workers: override > config > auto
    total_mem_gb = psutil.virtual_memory().total / 1e9
    hard_max = int(cfg_get(cfg, "runtime", "hard_max_workers", default=32))
    max_gb_per_worker = float(cfg_get(cfg, "runtime", "max_gb_per_worker", default=64))
    ...
    cfg_workers = int(cfg_get(cfg, "runtime", "workers", default=0))
    n_workers = args.workers if args.workers is not None else (cfg_workers or auto_workers)
