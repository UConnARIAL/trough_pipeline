...
CUTLINE_LAYER_NAME = "InputMosaicCutlinesVector"
from lib import get_cutline_vector_for_rasotor as CL
def _write_per_file_gpkg(tile_dir, tif_path, crs_wkt,
    ...
    logp(verbose, logging.DEBUG, f" STARTING PER FILE GPKG ADDING to n,e,c LAYER:{CUTLINE_LAYER_NAME}")  ###########################################
    # --- append extra layers to gpkg ---
    dst_layer = (CUTLINE_LAYER_NAME)
    # avoid clobbering core layer names accidently
    if dst_layer in {"GraphTheoraticNodes", "GraphTheoraticEdges", "GraphTheoraticComponents"}:
        dst_layer = f"ext_{dst_layer}"

    cutline_vec_path = CL.find_cutline_vector_path(tif_path=tif_path, tile_id=tile_id)

    if not cutline_vec_path:
        logging.warning(f"No cutline vector found for {tif_path}; skipping cutlines layer.")
    else:
        logging.debug(f"CUTLINE VECTOR {cutline_vec_path}")
        logging.debug(f"RASTER         {tif_path}")
        logging.debug(f"OUT GPKG       {out_gpkg}")
        logging.debug(f"DST LAYER      {dst_layer}")

        gdf = CL.clip_vector_to_raster_extent(tif_path, cutline_vec_path, fix_invalid=False)
        if gdf.empty:
            logging.info(f"Cutline clip produced 0 features; not writing '{dst_layer}' to {out_gpkg}.")
        else:
            gpkg_write_layer_replace(gdf, out_gpkg, dst_layer)
...
python 1_create_resume_subtile_gt_gpkg_.py \
       ...
       --output_tiles_dir ./TCN_refac_test_gpkg \
       --mosaic_gpkg ./TCN_refac_test_gpkg/all_mosaic_refac.gpkg \
