...
def aggregate_tile_from_perfile_gpkgs_OLD(tile_id, tile_dir, output_tiles_dir, verbose=False):
    ...
    if os.path.exists(gpkg_path): # REMOVE if already exists.
        os.remove(gpkg_path)
...
def _union_outer_polygon(gdf: gpd.GeoDataFrame):
    """Return a single (Multi)Polygon representing the outside boundary of the cutlines."""
    if gdf is None or gdf.empty:
        return None
    try:
        u = unary_union(gdf.geometry.values)
    except Exception:
        u = unary_union(gdf.buffer(0).geometry.values)
    # Ensure polygonal
    if u is None or u.is_empty:
        return None
    if u.geom_type in ("Polygon", "MultiPolygon"):
        return u
    # Fallback from possibly mixed types
    try:
        polys = [geom for geom in getattr(u, "geoms", []) if geom.geom_type in ("Polygon", "MultiPolygon")]
        if len(polys) == 1:
            return polys[0]
        if polys:
