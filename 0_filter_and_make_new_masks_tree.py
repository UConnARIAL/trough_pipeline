...
ANALYSIS_CRS = CRS.from_epsg(3413)
...
def load_water_geometry_albers(ocean_gpkg: Optional[str],
    ...
    if src_crs != ANALYSIS_CRS:
        to_alb = Transformer.from_crs(src_crs, ANALYSIS_CRS, always_xy=True).transform
...
def vector_ocean_fraction_albers(tile_bounds_src_crs, tile_crs, water_wkb_hex) -> float:
    minx, miny, maxx, maxy = transform_bounds(tile_crs, ANALYSIS_CRS, *tile_bounds_src_crs, densify_pts=21)
    ...
    try:
        with rasterio.Env(GDAL_CACHEMAX=gdal_cache_mb, GDAL_NUM_THREADS="1", **SAFE_ENV):
            with rasterio.open(tile_path) as ds:
                ...
                to_tile = Transformer.from_crs(ANALYSIS_CRS, ds.crs, always_xy=True).transform
...
"""
USAGE
python 0_filter_and_make_new_masks_tree.py \
  --tif-list logs/ca_ocean_test_tifs.txt \
  --in-root "$IN_ROOT" \
  --out-masks-dir "$TEST_ROOT" \
  --ocean-gpkg "$CA_OCEAN_GPKG" \
  --ocean-layer "$CA_OCEAN_LAYER" \
  --max-workers 1 \
  --link-mode symlink \
  --overwrite-existing \
  --out-csv logs/ca_ocean_filter_TEST.csv \
  --out-drop logs/ca_drop_tiles_TEST.txt \
  --out-boundary logs/ca_boundary_tiles_TEST.txt \
  --verbose
